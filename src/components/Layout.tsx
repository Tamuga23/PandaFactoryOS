...
const navigation = [
  ...
  { name: 'Store Purchases', href: '/purchases', icon: ShoppingBag },
  { name: 'Sales Records', href: '/history', icon: History },
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-zinc-950">
        ...
        <header className="hidden md:flex h-16 border-b border-zinc-800 items-center justify-between px-8 bg-zinc-900/20">
          <div className="flex items-center gap-4">
            <h2 className="text-lg font-medium text-zinc-200">Main System Overview</h2>
            <span className="px-2 py-1 bg-cyan-500/10 text-cyan-400 text-[10px] uppercase font-bold rounded border border-cyan-500/20">Cloud Connected</span>
          </div>
          <div className="flex items-center gap-6">
            <div className="relative cursor-help" title={`Updates stream in real-time.`}>
              {stats && stats.lowStockItems.length > 0 && (
            ...
            <div className="flex items-center gap-3 border-l border-zinc-800 pl-6">
              <div className="text-right hidden lg:block">
                <p className="text-sm font-medium text-zinc-200 leading-none">{user?.displayName || 'User'}</p>
