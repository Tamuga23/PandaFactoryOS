...
export default function Dashboard() {
  ...
  const kpis = [
    { title: 'Total Sales', value: formatCurrency(stats.totalSalesValue), icon: DollarSign, color: 'text-cyan-400', border: '' },
    { title: 'Inventory Value', value: formatCurrency(stats.totalStockValue), icon: TrendingUp, color: 'text-sky-400', border: '' },
    { title: 'Total Products', value: stats.totalProducts.toString(), icon: Package, color: 'text-white', border: '' },
    { title: 'Critical Stock', value: stats.lowStockItems.length.toString(), icon: AlertTriangle, color: 'text-rose-400', border: 'border-l-rose-500 border-l-2' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-x-auto p-0">
            {stats.lowStockItems.length > 0 ? (
                  ...
                  {stats.lowStockItems.map(item => (
                    <tr key={item.id} className="hover:bg-zinc-800/30 bg-rose-500/5">
                      ...
                      <td className="px-4 py-3 text-right">
                        <span className="px-2 py-0.5 rounded-full bg-rose-500/10 text-rose-500 text-[10px]">Low Stock</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <div className="p-5 text-sm text-zinc-500">All products are adequately stocked.</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden flex flex-col">
          <div className="p-4 border-b border-zinc-800 flex justify-between items-center">
            <h3 className="font-semibold text-zinc-200">Recent Sales</h3>
          </div>
          <div className="flex-1 overflow-x-auto p-0">
            {stats.recentSales.length > 0 ? (
              ...
              <div className="p-5 text-sm text-zinc-500">No recent sales.</div>
            )}
          </div>
        </div>
      </div>
